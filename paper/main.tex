...
For each workload, the runner records original size, output size for every attempted pass, accepted layer count, best pass, stopping reason, compression time, decompression time, platform string, and verification status. Verification decompresses the produced archive and compares the bytes with the original input. The runner writes per-pass and summary CSV files. The paper tables and plot data are generated from the CSV snapshots in \texttt{paper/data} by \texttt{paper/scripts/generate\_artifacts.py}.
...
\input{generated/passes-table.tex}

\begin{figure}[t]
\centering
\input{generated/trajectory-plot.tex}
